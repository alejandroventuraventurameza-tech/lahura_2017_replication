...
Los Gráficos 1 y 2 extendidos (código en el Anexo~A.10; export
pendiente de la corrida en EViews, ver nota al pie del Anexo~A.10)
muestran la misma comovimiento visual que en la muestra 2010--2017
para las tasas de mayor liquidez, pero con una dispersión bastante
mayor frente a la tasa de política en las series de plazos más largos
durante 2021--2023. Las correlaciones simples confirman esa lectura:
$r=0.98$ para la Preferencial 90 días hasta $r=0.35$ para Grandes
Empresas $>$360 días -- todas altamente significativas ($p<0.001$),
pero notablemente más bajas para R5--R8 que en la muestra original,
una primera señal (todavía descriptiva) de la inestabilidad que se
documenta en la Sección~2.4.
...
cointegra sin ambigüedad. Detalle completo, incluidos los rezagos por
serie, en el Anexo~A.10.
...
Medianas Emp.\ $>$360d   & 0.57 & $-0.01$\rlap{\textdagger} & $-0.13$ & (97.6)\tnote{a} & $\alpha$ marginal (10\,\%) \\
...
verificados en Python sobre los mismos 191 datos, pendientes de
confirmación final en EViews real (Anexo~A.10).
\item[a] El Promedio (meses) se reporta entre paréntesis y en cursiva
conceptual porque $\alpha$ no es significativo (R6) o solo lo es al
margen del 10\,\% (R7, R8, R9): dividir entre un $\alpha$
...
\textbf{Grandes Empresas $>$360 días (R6) el Cuadro 5 extendido no
produce un resultado interpretable}: el bloque sin restricciones da
$\beta_1=-3.55$ (signo económicamente absurdo) con una raíz de ajuste
positiva ($\alpha=0.008>0$, explosiva en vez de correctora), y el
bloque restringido (solo exogeneidad débil) diverge en vez de
converger a un valor razonable. La Sección~2.4 explica por qué.
...
10/5/1\,\%. Estadísticos verificados en Python; pendiente confirmación
en EViews real.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
de 72 meses (Gráfico~3, pendiente de exportar desde EViews, Anexo~A.10).
El ejemplo más claro es TAMN, cuyo $\beta_1$ rodante cae de 1.41 (en
ventanas centradas en 2016--2022) a 0.47 (ventana que termina en
junio de 2026); FTAMN muestra el patrón inverso, de 1.29 a 1.86.
...
series por igual. Un tratamiento riguroso y completo de esta
extensión requeriría, como mínimo, permitir un quiebre estructural
endógeno en la relación de cointegración misma para las series donde
el quiebre es más severo (R6, y en menor medida R7 y R8) --
