...
fueron generados en EViews real por Valeria Avilés y Andrea Quispe, a
partir de su propio código (\texttt{pregunta\_2/Outputs/p2.prg}, que
integro junto con el mío en el Anexo~A.10). Muestran la misma
comovimiento visual que en la muestra 2010--2017 para las tasas de
...
aporte de Valeria y Andrea que integro en el Anexo~A.10, Sección~8)
en vez de fijar un rezago común para las 9 series:
...
cada ecuación de corto plazo, Valeria y Andrea argumentan que el
\textbf{Caso 3} (\textit{unrestricted constant}) es el que corresponde
exactamente a esa estructura, y lo usan como especificación principal
en su propio informe, reportando el \textbf{Caso 2} (\textit{restricted
constant}, el mismo que usamos como principal en toda la Sección~1 y
en el resto de esta sección) como robustez. Mantengo el Caso 2 como
...
Lahura (2017) usó realmente -- pero incorporo el Caso 3 íntegro como
segunda columna, ya que en algunos casos (R1, TAMN) la conclusión
...
(diferencias $<2\,\%$, salvo R6--R7 -- ver Cuadro~5 extendido), pero
...
%%-------------------------------------------------------------
\subsection{Análisis de robustez: submuestra pre-pandemia (aporte de
            Valeria Avilés y Andrea Quispe)}
%%-------------------------------------------------------------

Todo lo anterior corresponde a la muestra completa 2010--2026. Pero
antes de llegar por mi cuenta al hallazgo de la Sección~2.4 (que R6 y
R7 se rompen específicamente por la ventana de tiempo que se analiza),
Valeria y Andrea ya habían corrido, en paralelo, el mismo ejercicio
completo restringido a 2010m08--2020m02 -- el último mes antes de la
emergencia sanitaria por COVID-19 en el Perú (declarada el 16 de marzo
de 2020) -- como chequeo de robustez independiente. Integro aquí sus
resultados casi tal cual los dejaron en su informe, porque llegan,
desde un ángulo distinto al mío, a la misma conclusión sobre R6 y R7,
y además porque su código de selección automática de rezagos por
Schwarz (que yo no había implementado) me pareció mejor que fijar un
rezago arbitrario para las nueve series, así que también lo adopté
para el Cuadro 3 extendido de la muestra completa (Cuadro~9 arriba).
...
\begin{tablenotes}[flushleft]
\footnotesize
\item * $p<0.10$; ** $p<0.05$; *** $p<0.01$. Números tal como los
confirmó Valeria en EViews real (su Cuadro 4, submuestra pre-pandemia).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}[flushleft]
\footnotesize\item
Este es, para mí, el resultado más importante que aporta la submuestra
de Valeria y Andrea: comparado con el Cuadro 5 extendido de la muestra
...
plazo). Esto es evidencia adicional, independiente de la mía en la
...
De esta comparación de tres muestras (que armaron originalmente
Valeria y Andrea, y que aquí solo actualizo con mi propia
nomenclatura de series) rescato tres lecturas. Primero, el traspaso de
las tasas de corto plazo (R1--R4) se mantiene entre 0.45 y 1.06 en las
tres muestras, siempre el más alto del grupo -- el hallazgo central de
\textcite{lahura2017} no depende de si se incluye o no la pandemia.
Segundo, el traspaso de las tasas de largo plazo (R5--R7) cae de forma
bastante consistente del paper (0.36--0.57) a la pre-pandemia
(0.18--0.48) y a la completa (0.27--0.54), lo que sugiero que refleja
un peso creciente de primas de riesgo de plazo cada vez menos ligadas
a la política monetaria de corto plazo. Tercero, TAMN y FTAMN son,
...
Vale la pena notar que este diagnóstico no depende únicamente de mi
propio ejercicio de sub-muestras (Gráfico 3, arriba): el análisis de
robustez pre-pandemia de la Sección~2.3, hecho de forma
independiente por Valeria y Andrea antes de que yo llegara a este
mismo punto por mi cuenta, apunta exactamente en la misma dirección
-- R6 y R7 son las únicas dos series cuyo VECM de muestra completa
deja de tener sentido económico y que, al recortar la muestra justo
antes de la pandemia, recuperan un $\beta_1$ de signo correcto
(Cuadro~15). Que dos ejercicios distintos, con particiones de muestra
distintas y hechos por separado, coincidan en señalar a las mismas dos
series es, para mí, la evidencia más convincente de todo este trabajo
de que el problema es real y no un artefacto de cómo corté yo la
